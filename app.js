...
// ═══════════════════════════════════════════
const STUDENTS = [
  {id:"SIT24SC017",name:"Aadhithya VS"},{id:"SIT24SC026",name:"Aashir Prajan A"},
  {id:"SIT24SC021",name:"Akilan K"},{id:"SIT24SC060",name:"Al Amaan H"},
  {id:"SIT24SC013",name:"Alamu Sridevi H"},{id:"SIT24SC051",name:"Anikasri B"},
  {id:"SIT24SC019",name:"Arshini Supritha S"},{id:"SIT24SC057",name:"Ashwin Perumal S R"},
  {id:"SIT24SC033",name:"Balavanitha R"},{id:"SIT24SC045",name:"Bharanidharan S"},
  {id:"SIT24SC047",name:"Bharath Kumar K"},{id:"SIT24SC006",name:"Deepanraj P"},
  {id:"SIT24SC043",name:"Devashri S"},{id:"SIT24SC003",name:"Devitha R"},
  {id:"SIT24SC032",name:"Dharani Sree M"},{id:"SIT24SC039",name:"Dhivyashri S"},
  {id:"SIT24SC014",name:"Hari Kumar E"},{id:"SIT24SC010",name:"Haswinth S"},
  {id:"SIT24SC011",name:"Isaki Ganesh M"},{id:"SIT24SC035",name:"Jagadeeshwaran S"},
  {id:"SIT24SC048",name:"Kamalesh K"},{id:"SIT24SC034",name:"Karthikeyan K"},
  {id:"SIT24SC028",name:"Kaviya A"},{id:"SIT24SC053",name:"Keshika K"},
  {id:"SIT24SC055",name:"Kirubalan M"},{id:"SIT24SC038",name:"Kirubavathy B"},
  {id:"SIT24SC058",name:"Kishore P"},{id:"SIT24SC036",name:"Kowsalya M"},
  {id:"SIT24SC002",name:"Manusri M V"},{id:"SIT24SC042",name:"Meena M"},
  {id:"SIT24SC007",name:"Mohamed Maajed M"},{id:"SIT24SC049",name:"Mohana Priya A"},
  {id:"SIT24SC015",name:"Mokshaya Durgai N"},{id:"SIT24SC027",name:"Naveen Adhithya M"},
  {id:"SIT24SC044",name:"Naveen P"},{id:"SIT24SC012",name:"Nithiswar S G"},
  {id:"SIT24SC022",name:"Piramanayagam P"},{id:"SIT24SC025",name:"Pooja Chettur S"},
  {id:"SIT24SC030",name:"Priyankha G"},{id:"SIT24SC037",name:"Punitha P"},
  {id:"SIT24SC031",name:"Raja Rajeswari A"},{id:"SIT24SC018",name:"Rakshita Pradhan"},
  {id:"SIT24SC052",name:"Rithika Shree J"},{id:"SIT24SC009",name:"Robin S"},
  {id:"SIT24SC050",name:"Sakravarthi R"},{id:"SIT24SC016",name:"Samarjeeth R"},
  {id:"SIT24SC024",name:"Sanjai S"},{id:"SIT24SC059",name:"Sanjeev Raj K"},
  {id:"SIT24SC041",name:"Santhosh K"},{id:"SIT24SC008",name:"Sreenidhi K"},
  {id:"SIT24SC001",name:"Srikanth R J"},{id:"SIT24SC056",name:"Srinidhi S"},
  {id:"SIT24SC023",name:"Sujitha J"},{id:"SIT24SC029",name:"Sulaka B"},
  {id:"SIT24SC004",name:"Surya D"},{id:"SIT24SC054",name:"Suryanarayanan D"},
  {id:"SIT24SC020",name:"Vishnuvarshan A"},{id:"SIT24SC046",name:"Yazhini G A"},
  {id:"SIT24SC005",name:"Yuvan Shankar B"},{id:"SIT24SC040",name:"Yuvasree P"},
  {id:"SITL25SC01",name:"Harish Gandhi"},{id:"SITL25SC02",name:"Arvind"},
];

let currentRoster = [...STUDENTS];
let uploadedRoster = null;
let activeRosterTab = 'default';
let autoDone = [], manual = [], missing = [], unknowns = [];
      ...
      lines.forEach((line, idx) => {
        ...
        if (parts.length >= 2) {
          ...
          if (match) {
             ...
             newRoster.push({ id: `UPL${String(newRoster.length+1).padStart(3,'0')}`, name: line });
          }
        }
      });
...
function updateUIForRosterChange() {
  autoDone = []; manual = []; missing = []; unknowns = [];
  document.getElementById('listDone').innerHTML = '<div class="flex items-center justify-center p-8 border-2 border-dashed border-outline-variant/30 rounded-xl opacity-40"><span class="font-label text-[10px] uppercase tracking-widest">Run analysis first</span></div>';
  document.getElementById('listMiss').innerHTML = '<div class="flex items-center justify-center p-8 border-2 border-dashed border-outline-variant/30 rounded-xl opacity-40"><span class="font-label text-[10px] uppercase tracking-widest">Run analysis first</span></div>';
...
function run() {
  ...
  if (!chat.trim()) { alert('Please paste the WhatsApp chat export first!'); return; }
  ...
  const re = /\[\d{1,2}\/\d{1,2}\/\d{2,4},\s*\d{1,2}:\d{2}(?::\d{2})?\s*(?:AM|PM)?\] ([^:]+):/gi;
  const senders = new Set();
  let m;
  while ((m = re.exec(filteredChat)) !== null) senders.add(m[1].trim());
  senders.forEach(s => { if (/^[\+\d][\d\s\-\(\)]{5,}$/.test(s)) unknowns.push(s); });

  currentRoster.forEach(s => {
    const parts = s.name.toLowerCase().split(' ').filter(p => p.length > 2);
  ...
  if (unknowns.length) {
    ...
    document.getElementById('unknownPills').innerHTML = unknowns.map(n =>
...
function updateStats() {
  ...
  document.getElementById('sManual').textContent = String(manual.length).padStart(2,'0');
...
function render() {
  ...
  const fDone = autoDone.filter(s => s.name.toLowerCase().includes(q) || s.id.toLowerCase().includes(q));
  const fMiss = missing.filter(s => s.name.toLowerCase().includes(q) || s.id.toLowerCase().includes(q));
  const fManual = manual.filter(s => s.name.toLowerCase().includes(q) || s.id.toLowerCase().includes(q));
...
// MARK / UNMARK
// ═══════════════════════════════════════════
function markDone(i) { manual.push(missing[i]); missing.splice(i,1); updateStats(); render(); }
function unmark(i) { missing.push(manual[i]); manual.splice(i,1); updateStats(); render(); }

// ═══════════════════════════════════════════
// COPY LIST
// ═══════════════════════════════════════════
function copyList(type) {
  const list = type === 'done' ? [...autoDone,...manual] : type === 'miss' ? missing : manual;
  const txt = list.map((s,i) => `${i+1}. ${s.name} (${s.id})`).join('\n');
  navigator.clipboard.writeText(txt).then(() => {
    const btnId = type === 'done' ? 'copyDone' : type === 'miss' ? 'copyMiss' : 'copyManual';
    const btn = document.getElementById(btnId);
    const icon = btn.querySelector('.material-symbols-outlined');
    icon.textContent = 'done';
    btn.classList.add('bg-secondary/10');
    setTimeout(() => { icon.textContent = 'content_copy'; btn.classList.remove('bg-secondary/10'); }, 2000);
  });
}
...
function renderRosterPage() {
  ...
  if (btnDef && btnUpl) {
    if (activeRosterTab === 'default') {
      btnDef.className = "px-5 py-2 rounded-lg font-label text-xs uppercase tracking-widest text-on-primary bg-primary shadow-sm transition-all";
      btnUpl.className = "px-5 py-2 rounded-lg font-label text-xs uppercase tracking-widest text-on-surface-variant hover:text-on-surface hover:bg-surface-variant transition-all";
    } else {
      btnUpl.className = "px-5 py-2 rounded-lg font-label text-xs uppercase tracking-widest text-on-primary bg-primary shadow-sm transition-all";
      btnDef.className = "px-5 py-2 rounded-lg font-label text-xs uppercase tracking-widest text-on-surface-variant hover:text-on-surface hover:bg-surface-variant transition-all";
    }
  }

  let listToRender = (activeRosterTab === 'default') ? STUDENTS : (uploadedRoster || []);
  const grid = document.getElementById('rosterGrid');
  if (!grid) return;

  if (activeRosterTab === 'uploaded' && (!uploadedRoster || uploadedRoster.length === 0)) {
    grid.innerHTML = '<div class="col-span-full flex flex-col items-center justify-center p-12 opacity-50"><span class="material-symbols-outlined text-4xl mb-3">upload_file</span><span class="font-label text-xs uppercase tracking-widest">No custom roster uploaded</span></div>';
    return;
  }

  const searchEl = document.getElementById('rosterSearch');
  const q = searchEl ? searchEl.value.toLowerCase() : '';
  const filtered = listToRender.filter(s => s.name.toLowerCase().includes(q) || s.id.toLowerCase().includes(q));
  ...
  grid.innerHTML = filtered.map(s => {
    ...
    if (autoDone.find(x => x.id === s.id)) {
      statusBadge = '<span class="inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-secondary-container text-on-secondary-container text-[10px] font-label uppercase tracking-widest"><span class="material-symbols-outlined text-xs">check</span>Sent</span>';
      borderExtra = 'border-secondary/30';
    } else if (manual.find(x => x.id === s.id)) {
      statusBadge = '<span class="inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-primary-fixed text-on-primary-fixed-variant text-[10px] font-label uppercase tracking-widest"><span class="material-symbols-outlined text-xs">edit</span>Marked</span>';
      borderExtra = 'border-on-primary-fixed-variant/30';
    } else if (missing.find(x => x.id === s.id)) {
      statusBadge = '<span class="inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-error-container text-on-error-container text-[10px] font-label uppercase tracking-widest"><span class="material-symbols-outlined text-xs">close</span>Missing</span>';
  ...
  if (!filtered.length) {
    grid.innerHTML = '<div class="col-span-full flex items-center justify-center p-12 opacity-40"><span class="font-label text-xs uppercase tracking-widest">No students match your search</span></div>';
  }
}
...
function refreshAnalytics() {
  const total = currentRoster.length;
  const hasData = autoDone.length > 0 || manual.length > 0 || missing.length > 0;

  // Toggle no-data state
  document.getElementById('analyticsNoData').classList.toggle('hidden', hasData);
  ...
  document.getElementById('aManual').textContent = String(manual.length).padStart(2,'0');
  document.getElementById('aMissing').textContent = missing.length;
  document.getElementById('aRateBig').textContent = pct + '%';
  setTimeout(() => { document.getElementById('aRateBar').style.width = pct + '%'; }, 100);
  ...
  setTimeout(() => {
    document.getElementById('aAutoFill').style.width = (autoDone.length/total*100) + '%';
    document.getElementById('aManFill').style.width = (manual.length/total*100) + '%';
    document.getElementById('aMisFill').style.width = (missing.length/total*100) + '%';
  }, 100);

  // Missing students list
  const ml = document.getElementById('aMissingList');
  if (missing.length) {
    ml.innerHTML = missing.map(s => {
      const colorClass = getAvatarColor(s.name);
      return `
    ...
    ml.innerHTML = '<div class="flex items-center justify-center p-8"><span class="font-label text-[10px] uppercase tracking-widest text-secondary">🎉 All students responded!</span></div>';
  }
}
